...
def _login_admin(client, admin_id, join_code):
    ...
    economy = ClassEconomy.query.filter_by(join_code=join_code, user_id=user_id).first() if user_id else None
...
def _setup_shared_student_with_split_membership():
    ...
    class_a = ClassEconomy(join_code="TAPA01", user_id=admin_a.user_id, status="active")
    class_b = ClassEconomy(join_code="TAPB01", user_id=admin_b.user_id, status="active")
    db.session.add_all([class_a, class_b])
    db.session.flush()

    user_a = db.session.get(User, admin_a.user_id)
    user_b = db.session.get(User, admin_b.user_id)
    ...
    db.session.add_all([teacher_seat_a, teacher_seat_b])
    db.session.flush()
