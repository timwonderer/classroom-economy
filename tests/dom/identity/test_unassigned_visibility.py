...
def test_new_admin_cannot_see_unassigned_students(client):
    ...
    """
    teacher_b = seed_canonical_admin("teacher_b_leak", pyotp.random_base32()).user
    db.session.flush()
    ...
    teacher_a = seed_canonical_admin("teacher_a_leak", pyotp.random_base32()).user
    db.session.flush()
    db.session.commit()
...
def test_owner_can_see_students_in_own_class(client):
    ...
    """
    teacher_b = seed_canonical_admin("teacher_b_owner", pyotp.random_base32()).user
    db.session.flush()
