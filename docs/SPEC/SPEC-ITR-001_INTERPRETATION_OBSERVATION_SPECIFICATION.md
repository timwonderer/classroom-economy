...
### 3.1 Computation Metadata: Normalization Dependency

This specification introduces one additional declared property beyond `DOM-ITR-001` §IV's three, applicable to outputs that scale their result by a class-configuration derivative:

- **Normalization dependency** — the configuration derivative (if any) used to rescale the output's units. Declared with its authoritative source. `CWI` per `SPEC-ECON-003 §4.1` is the only value used in v1.0.

Normalization is distinct from Reference Dependency. A normalized output does not compare an observation to an expectation; it reports the same observation in different units. An output that declares a Normalization dependency still declares Reference Dependency independently. Per INV-ITR-014, a normalization value SHALL NOT be treated as an observational expectation.
...
- **Semantic Kind:** Descriptive observation
- **Subject:** `class_id`
- **Observation basis:** `Transaction.id`, filtered to student-originated transfers with `account_type='savings'` on the deposit side of a correlated transfer pair (per `DOM-LED-001` INV-LED-007)
