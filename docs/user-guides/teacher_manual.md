---
title: Teacher Manual
description: Quick index of teacher troubleshooting and feature guides.
category: user-guides
roles: [teacher]
---

# Teacher Manual (Quick Index)

This page is a shortcut list. Use diagnostics for fast troubleshooting and feature guides for step-by-step tasks.

## Start here
- [Teacher Help and Support Guides](diagnostics/teacher.md)

## Common questions
- [I cannot log in or reset my account](diagnostics/teacher/login.md)
- [A feature is missing for students](diagnostics/teacher/onboarding.md)
- [Students cannot claim or join a class](diagnostics/teacher/students.md)
- [Payroll or attendance looks wrong](diagnostics/teacher/attendance-payroll.md)
- [Balances, transfers, or interest look wrong](diagnostics/teacher/transactions-banking.md)
- [Store items are missing or redemptions are stuck](diagnostics/teacher/store.md)
- [Rent or insurance settings are not behaving](diagnostics/teacher/rent-insurance.md)
- [Hall passes are stuck or not approving](diagnostics/teacher/hall-pass.md)
- [Announcements or issue reports are missing](diagnostics/teacher/announcements-issues.md)
- [Analytics dashboard not loading or showing wrong data](diagnostics/teacher/analytics.md) **(v1.7+)**
- [Rent itemization or privilege badges not working](diagnostics/teacher/rent-itemization.md) **(v1.7+)**

## Feature Guides (Structured by Section)
- [Teacher Feature Guides Index](features/teacher/index.md)
- [Classroom Setup and Students](features/teacher/classroom/class-setup.md)
- [Attendance and Payroll](features/teacher/economy/payroll-run.md)
- [Store and Redemptions](features/teacher/economy/store-items.md)
- [Economic Engine](features/teacher/economy/economic-engine.md)
- [Economic Policy and Rebalancing](features/teacher/economy/policy-mode-rebalancer.md)
- [Rent and Insurance](features/teacher/bills/rent-settings.md)
- [Customizations](features/teacher/settings/customizations.md)
- [Economy Features](features/teacher/settings/economy-features.md)
