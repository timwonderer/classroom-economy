---
title: Teacher Help and Support Guides
description: Index of teacher troubleshooting guides, grouped by the part of the app that looks wrong.
category: diagnostics
roles: [teacher]
---

# Teacher Help and Support Guides

Here are some issues that teachers typically encounter.




- [Login and Account Security](teacher/login.md)
- [Onboarding and Feature Settings](teacher/onboarding.md)
- [Students and Join Codes](teacher/students.md)
- [Attendance and Payroll](teacher/attendance-payroll.md)
- [Transactions and Banking](teacher/transactions-banking.md)
- [Store and Redemptions](teacher/store.md)
- [Rent and Insurance](teacher/rent-insurance.md)
- [Hall Passes](teacher/hall-pass.md)
- [Announcements and Issues](teacher/announcements-issues.md)
- [Analytics Dashboard](teacher/analytics.md)
- [Rent Itemization](teacher/rent-itemization.md)

## Direct Support Tickets

Teachers can now submit class-scoped support tickets directly from **Help & Support** (`/admin/help-support`). Select the class scope (join code), describe the issue, and the ticket is delivered directly to system administration without a teacher triage step. Note that you must have at least one class set up (with a join code) before you can submit a direct support ticket; if you have no classes yet, create a class first (see [Students and Join Codes](teacher/students.md)).
