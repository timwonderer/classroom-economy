...
- [/] Wave 11 FEATBypass default-flip — invert `tests/conftest.py` so FEAT enforcement is the test default and `FEATBypass` is opt-in per test. **Phase 1 (instrumentation) complete 2026-06-09. Phase 2 (fixture consolidation) has landed a shared helper catalog and several canonical test migrations, with the policy-mode and collective-goal implied-authority audit gaps now resolved; any remaining work is fixture-helper cleanup outside those slices.** Full plan in [V2_FEAT_BYPASS_DEFAULT_FLIP_PLAN.md](./V2_FEAT_BYPASS_DEFAULT_FLIP_PLAN.md); Phase 1 findings in [V2_FEAT_BYPASS_DEPENDENCY_REPORT.md](./V2_FEAT_BYPASS_DEPENDENCY_REPORT.md). The four previously named dead mutating endpoints have now been verified FEAT-owned in the live tree (`admin.process_claim`, `sysadmin.resolve_escalated_issue`, `admin.rent_settings`, `admin.passkey_auth_finish`), so the remaining work is fixture-seeding consolidation and enforcement default inversion rather than route decoration. Original trigger: 2026-06-08 audit of `/api/approve-redemption` and `/api/reject-redemption` (dead, now fixed via `FEAT-STOR-006`) and the cross-FEAT correlation bug in `Transaction.before_update` (also fixed).
...
- Session note 2026-07-12: repaired the next bounded FEATBypass slice by normalizing `/api/set-timezone` to return a valid unauthorized response, removing raw commits from dashboard-rendering setup, and eliminating the student-dashboard crash on missing legacy block metadata. Focused verification passed for `tests/test_api_fixes.py` and `tests/test_dashboard_rendering.py`, with the structured idempotency-key warning class removed from these slices.
...
- Session note 2026-07-13: audited the commit range starting at `08eb6318` for canonical v2 compliance. Rent-display route timing now uses class timezone, rent-display tests use canonical class/student helpers, and the policy-mode / collective-goal implied-authority fixture gaps were resolved in [`docs/TRACKING/AUDIT_IMPLIED_AUTHORITY_TODO.md`](./AUDIT_IMPLIED_AUTHORITY_TODO.md). Targeted validation passed for the repaired slices.
