...
- Session note 2026-07-06: cleaned the admin issue-resolution boundary in `app/routes/admin.py` so the ticket workflow uses owner/admin wording in its local names and messages while keeping the FEAT/status contract unchanged. Also tightened `scripts/seed_canonical_v2.py` to keep its legacy compatibility shadow explicit after the import cleanup. Validation stayed clean with `venv/bin/python -m py_compile app/routes/admin.py scripts/seed_canonical_v2.py` and `venv/bin/flask --app wsgi routes >/dev/null && echo OK`.
