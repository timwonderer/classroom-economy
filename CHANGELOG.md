...
- **Phase 1 Jinja2 Remediation In Progress (2026-08-07)** — Implemented Phase 1 of SPEC-UI-001 template remediation for Store, Obligations, and Payroll domains. Implemented domain-specific view model builders: `StoreItemCardView`, `EntitlementCardView`, `CollectiveProgressView` (store); enhanced `StudentObligationView`, `ClassObligationSummary` (obligations); `StudentPayrollStatusView`, `PayrollConfigurationView` (payroll). Updated routes to build and pass view models instead of raw ORM objects. COMPLETED: `student_shop.html` (37% reduction, all formatting moved to builders). IN PROGRESS: `admin_rent_settings.html` and `admin_payroll.html` still contain Jinja2 strftime() and |format() expressions (moved to backlog). Fixed violations on student_shop: Pattern 1 (numeric formatting) → pre-formatted display strings; Pattern 2 (ORM `.strftime()`) → pre-computed dates; Pattern 4 (business logic) → view model computation; Pattern 6 (ORM traversal) → flattened properties. Status: 1/3 templates complete. Commits: 9103ded8, ffd76ad2, 942a7342.
