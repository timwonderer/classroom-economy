...
- **Phase 1 Jinja2 Remediation Complete (2026-08-07)** — Completed Phase 1 of SPEC-UI-001 template remediation for Store, Obligations, and Payroll domains. Implemented domain-specific view model builders: `StoreItemCardView`, `EntitlementCardView`, `CollectiveProgressView` (store); enhanced `StudentObligationView`, `ClassObligationSummary` (obligations); `StudentPayrollStatusView`, `PayrollConfigurationView` (payroll). Updated routes to build and pass view models instead of raw ORM objects. Remediated templates eliminate 30+ lines of business logic and ORM traversal; all Jinja2 currency/date formatting moved to builders. Fixed violations: Pattern 1 (numeric formatting) → pre-formatted display strings; Pattern 2 (ORM `.strftime()`) → pre-computed dates; Pattern 4 (business logic) → view model computation; Pattern 6 (ORM traversal) → flattened properties. Templates fixed: `student_shop.html` (37% reduction), `admin_rent_settings.html` (student count pre-computation), `admin_payroll.html` (balance pre-formatting). Commits: 9103ded8, ffd76ad2, 942a7342.
